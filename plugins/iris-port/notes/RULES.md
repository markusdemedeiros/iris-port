...
- The FIRST GOLDEN RULE: You are never permitted to use git. git, or any version control command, is expressly forbidden under any circumstance. 
...
- Use the `iris-rocq-expert` subagent to query the Rocq source. It uses Rocq-MCP to inspect proof states when a working Rocq install is available, and otherwise reads/greps the source directly.
...
- The ideal code matches the style of existing code in the repository. 
...
- The key rules of Lean code review:
    + One line equals one idea. Do not splice tactics together with semicolons artificially.
